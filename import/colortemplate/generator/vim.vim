...
const NO_DISCR_VALUE = lib.DEFAULT_DISCR_VALUE
...
def AttributesToString(t: dict<any>, meta: dict<any>, termgui: bool): list<string>
  ...
  for key in ['Fg', 'Bg', 'Special']
    if !empty(meta[key]) && !empty(t[key])
      ...
      if termgui
        var colorValue: string = meta.GuiColors.Colors[t[key]]
      ...
      var colorValue: string = meta.Colors[t[key]]
...
def BaseGroupToString(t: dict<any>, meta: dict<any>, indent = 0, termgui = false): string
  ...
  var attrs = AttributesToString(t, meta, termgui)

  if empty(attrs)
    return null_string  # Skip definition
  endif

  var hiGroupDef = ['hi', t.HiGroupName] + attrs
...
export class Generator implements lib.Generator
  ...
  def Header(): list<string>
    ...
    var license = empty(theme.license) ? 'Vim License (see `:help license`)' : theme.license
    ...
    if theme.options.creator
      header->AddMeta('%s Generated by Colortemplate v%s', this._comment, lib.VERSION)
      header->add('')
    endif
  ...
  def Footer(): list<string>
    ...
    if theme.options.palette
      var commentSymbol = theme.options.backend == 'legacy' ? '"' : '#'
  ...
  def Emit(background: string, indent = 0): list<string>
    ...
    var globalLinked = db.LinkedGroup->Sort(CompareByHiGroupName)
    var globalBaseGr = db.BaseGroup  ->Sort(CompareByHiGroupName)
  ...
  def GenerateVariant(background: string, variant: string, indent: number, onlyOverrides = false): list<string>
    ...
    var variantMeta = db.GetVariantMetadata(variant)
    var nextIndent  = indent + this._shiftwidth
  ...
  def GenerateOverridingDefinitions(background: string, variantMeta: dict<any>, indent: number): list<string>
    ...
    var discrNames: list<string>          = keys(overrides)->sort()
    var defs:       list<string>          = []
    var space:      string                = repeat(' ', indent)

    for discrName in discrNames
      var hiGroups = overrides[discrName]->sort(CompareByDiscrValueAndHiGroupName)
      ...
      for t in hiGroups
        ...
        var overridingDef = HiGroupToString(t, variantMeta, indent + this._shiftwidth, variantMeta.Variant != 'gui')
  ...
  def EndVariant(variantMeta: dict<any>, background: string, indent: number): list<string>
    ...
    var doublespace = space .. repeat(' ', this._shiftwidth)
    var output: list<string> = []

    if this._backend == 'legacy'
      var db = this.theme.Db(background)
      var discriminators = db.Discriminators()
