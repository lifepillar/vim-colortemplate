...
fun! s:print_header(bufnr)
  ...
  if s:getopt('timestamp')
    call s:put(a:bufnr,         '" Last Updated: ' . strftime(get(g:, 'colortemplate_date_format', '%Y %b %d')))
  endif
  if s:getopt('creator')
    call s:put(a:bufnr,         '')
  ...
  if s:supports_neovim() || s:getopt('backward_compatible')
    " In Neovim, t_Co is deprecated. The appropriate way to use less than 256
    " colors is to instruct the color scheme to do so.
    " See also: https://github.com/neovim/neovim/issues/11974
    let l:prefix = s:info['optionprefix']
    call s:put(a:bufnr, printf(
...
fun! s:print_footer(bufnr)
  call s:put(a:bufnr, s:getopt('use_tabs') ? '" vim: noet ts=8 sw=8 sts=8' : '" vim: et ts=8 sw=2 sts=2')
endf
